...
def set_cbar(image,axis,cbar_label):
    ...
    cb.ax.tick_params(which='major',direction='in',length=5)
    cb.ax.tick_params(which='minor',direction='in',length=3)
    return None
...
data = analyze.readData(gdens=True,ddens=plot_dust,ispec=mname, gtemp=plot_temp,gvel=False,binary=False)
...
if plot_mol:
    ...
    ax[0].imshow(np.log10(Nh2x[:, :, 0]), origin='lower') #extent=[0, box/pc, 0, box/pc])
    ax[1].imshow(np.log10(Nh2y[:, :, 0]), origin='lower') #extent=[0, box/pc, 0, box/pc])
    #ax[0].set(xlabel='L [pc] ', ylabel='L [pc]')
    im = ax[2].imshow(np.log10(Nh2z[:, :, 0]), origin='lower') #extent=[0, box/pc, 0, box/pc])
    #ax[0].set(xlabel='L [pc] ', ylabel='L [pc]')
    ax[1].set(title='H$_2$ Column Density')
    set_cbar(im, ax[2], 'log N_H2')

    if save == True:
        plt.savefig("ColH2_dist_check.png")
    else:
        plt.show()
    print("Max log Nh2", np.log10(np.max(Nh2x)), np.log10(np.max(Nh2y)), np.log10(np.max(Nh2z)))
    print("Min log Nh2", np.log10(np.min(Nh2x)), np.log10(np.min(Nh2y)), np.log10(np.min(Nh2z)))
