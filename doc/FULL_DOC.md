...
### image.py
Contains functions for handling image.out files, including calculating and plotting moment maps. 
Replaces image.py in the radmc3dPy python tools directory.
...
1. Install radmc3d (See https://www.ita.uni-heidelberg.de/~dullemond/software/radmc-3d/manual_radmc3d/):
    git clone https://github.com/dullemond/radmc3d-2.0.git
    cd src
    make
2. Install yt (https://yt-project.org) 
3. Install yt tools extension: 
    pip install yt-astro-analysis
5. Clone this repository on your local machine.
6. Add yt and carver tools to your path: export PATH=$HOME/yt-conda/lib/python3.8/site-packages:$HOME/yt-conda/bin:$HOME/gizmo_carver/src:$PATH
8. Copy src/image.py into the radmc3dPy tools directory, replacing the exisiting image.py there.
9. In radmc3dPy directory run: python setup.py install 
10. Create/specify a directory to store the output files.
11. Locate the desired GIZMO dataset file.
12. Update the `inputs_gizmo_carver.py` file.
    ...
    2. Update the `hdf5_file` parameter to give the correct filepath to the GIZMO
    ...
    3. Update the `existing_filepath` parameter to give the correct filepath to the
    ...
    The default files are as follows:
        ...
        ```
    4. Update the `output_filepath` parameter with the filepath of the output directory
    mentioned earlier.
    
13. After the `inputs_gizmo_carver.py` file has been fully updated, run the 
...
14. After RADMC-3D has generated the output file `image.out`, use `radmc_image_processing.py` to 
